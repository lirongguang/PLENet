...
from models import EdgeAggregatedGAT_attention_visual_EGNN as egret
...
class EgretPPI(nn.Module):
    def __init__(self):
        ...
        self.conv_encoder = nn.Sequential(
            nn.Conv1d (
                in_channels=configs.num_esm_features + configs.num_phychem_features + configs.num_residue_accessibility_features,
                out_channels=32,
                kernel_size=7, 
                stride=1,
                padding=7 // 2, 
    ...
    def forward (self, protbert_feature, graph_batch, phychem_data, residue_accessibility_data, coordinates,  max_sequence_length=configs.max_sequence_length):
        merged_features = torch.randn(0).to(device)
    ...
    def forward(self,
                protbert_data_receptor, graph_batch_receptor, phychem_data_receptor, residue_accessibility_data_receptor,
                    protbert_data_ligand, graph_batch_ligand, phychem_data_ligand, residue_accessibility_data_ligand,
                        pair_residue_label, receptor_coordinates, ligand_coordinates):
