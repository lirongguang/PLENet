...
from models.Pair_EGRET_EGNN import PairEgret
import data_generator_attention_visual_ESM_coordinates as data_generator
from data_generator_attention_visual_ESM_coordinates import GraphCollate
...
def train_epoch(model, loader, optimizer, epoch, all_epochs):
    ...
    for batch_idx, batch_data in enumerate(loader):
        # Modified: unpack residue coordinate data as well
        ...
        complex_info_batch, pair_residue_label, ligand_label_batch, receptor_label_batch, \
        receptor_coordinates, ligand_coordinates = batch_data

        protbert_data_receptor = torch.autograd.Variable(protbert_data_receptor.to(device).float())
        graph_batch_receptor = graph_batch_receptor.to(device)
        ...
        # Modified: handle receptor residue coordinates
        receptor_coordinates = torch.autograd.Variable(receptor_coordinates.to(device).float())

        protbert_data_ligand = torch.autograd.Variable(protbert_data_ligand.to(device).float())
        graph_batch_ligand = graph_batch_ligand.to(device)
        ...
        # Modified: handle ligand residue coordinates
        ligand_coordinates = torch.autograd.Variable(ligand_coordinates.to(device).float())
        ...
        # Modified: pass residue coordinates into the model
        ...
        if batch_idx % configs.print_freq == 0:
            ...
            for (output_name, output) in [('receptor_pred', receptor_pred), ('ligand_pred', ligand_pred),
                ...
                output = output.numpy()
                auroc, auprc, accuracy, f1, p, r, threshold, mcc = basic_metrics(output, label)
                res = '\t'.join([
...
def eval_epoch(model, loader, save_attention_scores=False):
    ...
    with torch.no_grad():
        for _, batch_data in enumerate(loader):
            ...
            # Modified: pass residue coordinates into the model
            receptor_pred, ligand_pred, pairwise_pred, head_attn_scores = model(
                protbert_data_receptor,
                graph_batch_receptor,
                phychem_data_receptor,
                residue_accessibility_receptor,
    ...
    for (output_name, output, true_label, weights) in [('receptor_pred_all', receptor_pred_all, label_all, weight),
        ...
        auroc, auprc, accuracy, f1, p, r, threshold, mcc = basic_metrics(output, true_label)
        ...
        results += [[output_name, median_auroc, auroc, auprc, accuracy, f1, p, r, val_loss, threshold, mcc]]
...
def run_experiment(root_dir, total_epochs, save_dir, experiment_num, model_file):
    ...
    f.write('epoch,output_name,median_auroc,auroc,auprc,accuracy,f1_score,precision,recall,val_loss,train_loss,best_threshold,mcc\n')
    ...
    for epoch in range(total_epochs):
        ...
        for _, (output_name, median_auroc, auroc, auprc, accuracy, f1, p, r, val_loss, threshold, mcc) in enumerate(results):
            f.write('{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(epoch, output_name, median_auroc, auroc, auprc,
                                                              accuracy, f1, p, r,
                                                              val_loss, train_loss, threshold, mcc))
    ...
    if configs.PREDICT_INTERFACE_REGION:
        ...
        for (outputs, labels, output_name) in result_pairs:
            auroc, auprc, accuracy, f1, p, r, threshold, mcc = basic_metrics(outputs, labels)
...
if __name__ == '__main__':
    ...
    # New: residue coordinate related arguments
